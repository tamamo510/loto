...
### ブランチ状態
- **main**: v7.4.2（CMA-ES + Weibull + Confidence修正済み）
- **claude/review-progress-readme-3F3rG**: v7.5-cold-wave（**PR #27 未マージ**）
- **マージ待ちPR**: #27

### PR #27 の内容（要レビュー・マージ）
1. `coldWave(num, draws)` — Wave 9番目
   - Z-score（二項分布）: Z < -2.5 → -15pt, Z < -2.0 → -10pt, Z < -1.5 → -5pt
   - 最大ギャップ検出: currentGap ≥ maxHistGap×0.9 かつ expectedGap×3倍超 → -12pt
   - 出力: [-20, +5] × coldMult（CMA-ES対象）
2. UI追加（Prediction タブ）: 削除候補数字ボックス（🚫 score≤-10 / ▲ -10〜-5）
3. UI追加（Waves タブ）: Cold Number Analysis パネル（全数字スコア一覧）
4. UI追加（Backtest タブ）: ランダム基準線・v7.4.2参照値との比較行
5. `savePredictionToLog`: バージョン文字列を `GLEF_VERSION` 定数参照に修正

### PR #27 で確認すべき点
- [ ] ブラウザ実機確認（削除候補UI表示）
- [ ] coldWave の AvgHit 1.80 が妥当か検証（過大評価の疑い）
- [ ] 全バックテスト（直近/全期間/異常回後）でLoto6・Loto7とも数値確認
- [ ] CMA-ES実行後のcoldMult値確認（0.2〜2.5の範囲内か）

### 残タスク（優先順）
1. **PR #27 マージ確認・動作検証**（最初にやること）
2. **クロスロト引っ張り実装** (`crossLotoBias` Wave) — 高優先
   - Loto6↔Loto7間の直近当選数字引っ張り理論
   - 根拠: R2089 Loto6の{09,18,37}がLoto7 R670と一致
   - data.jsに両データあり、即実装可能
3. **セット球 → setWave実装** — 即実装可能
   - PDFパース検証済み
   - 9番目のWave `setWave` + CMA-ES `setMult`
4. **Loto6精度改善調査** — AvgHits 0.85 ≈ ランダム基準
5. **glef_predict.js v7.5対応**（v7.3のまま）
6. **Confidenceスケーリング微調整**（新理論実装後に再評価）

### 作業ルール（次スレ厳守）
- **毎回コミット→プッシュ→PR作成**（PR省略禁止）
- **PROGRESS記録は作業完了時に即更新**（「→後で確認」禁止）
- **バックテスト数値は必ず記録**（スクリーンショットの数値をそのまま記載）
- コンテキスト圧縮を起こさないこと
- 外部サイトのデータ使用禁止
- データリーク厳禁（N-1ドローズのみ）
