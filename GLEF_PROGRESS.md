...
### Confidence計算（v7.4.1修正）
```
conf = avgHit/pick * 40 + prizeRate * 30 + hitRate/100 * 30
```
バックテスト実績ベース。スコア分離度ではない。
...
### ブランチ状態
- **ブランチ**: `claude/create-pr-v7.3-updates-n6MeY` → main
- **リモートpush済み**
- **mainからの差分コミット**:
  | コミット | 内容 |
  |---------|------|
  | `b791310` | v7.4: CMA-ES + ワイブル異常回確率インジケーター |
  | `54c0e59` | Confidence計算をバックテスト実績ベースに修正 |

### 残タスク
1. **PR作成 → マージ**: `claude/create-pr-v7.3-updates-n6MeY` → main
   - PR作成がこのスレで完了していない場合に実施
2. **Loto6精度改善調査**: Avg Hits 0.85がランダム基準(~0.84)とほぼ同じ。波形関数のLoto6対応を検討
3. **glef_predict.js のv7.4対応**: Node.js予測エンジンがv7.3のまま。CMA-ES/anomalyRisk未対応
