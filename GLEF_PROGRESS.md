...
#### 2. セット球パターン分析 — **データ整備後**
- **データ**: sougaku.com/loto7/data/list1/ に全回分あり（回号＋当選番号＋セットA~J）
- **取得方法**: WebFetch/スクレイピングは403でブロック。手動コピペ→スプレッドシート→CSV化が必要
- **現状**: ユーザーが100回分を手動コピペ可能なことは確認済み（スプレッドシートのスクショあり）
- **実装案**: 9番目のWave「setWave」＋CMA-ESの`setMult`追加
- **ブロッカー**: データ整備がユーザーの手動作業。完了後に実装着手
...
- **マージ済みPR**: #21（v7.4 CMA-ES + anomaly risk + confidence v7.4.1）、#22（Confidence v7.4.2 スケーリング修正）

### 残タスク（優先順）
1. **削除数字（自力導出版）実装**: 当選データのみから除外数字を統計導出。新Wave or 既存Wave負側拡張
2. **セット球データ整備 → setWave実装**: ユーザーが手動CSV化完了後
3. **Loto6精度改善調査**: Avg Hits 0.85 ≈ ランダム基準(~0.84)
4. **glef_predict.js のv7.4対応**: Node.js予測エンジンがv7.3のまま
5. **Confidenceスケーリング微調整**: 新理論実装後に再評価
